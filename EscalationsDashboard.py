...
def process_uploaded_file(file):
    try:
        ...
        with st.spinner('Processing your data...'):
            ...
            # Include both TA and Census subject line variants
            expected_headers = [
                "Mode",
                "Type",
                "Escalation Date",
                ...
                "BID",
                "Employer Name",
                # Include both subject line variants
                "Subject line (Manual TA Escalation)",
                "Subject line (Manual census Escalation)",
                "Parent Category",
                "Case Category",
                "Escalated To"
            ]
            
            # Determine if this is a TA or Census escalation file
            if "Subject line (Manual TA Escalation)" in df.columns and "Subject line (Manual census Escalation)" not in df.columns:
                st.session_state['file_type'] = 'TA'
            elif "Subject line (Manual census Escalation)" in df.columns and "Subject line (Manual TA Escalation)" not in df.columns:
                st.session_state['file_type'] = 'Census'
            elif "Subject line (Manual TA Escalation)" in df.columns and "Subject line (Manual census Escalation)" in df.columns:
                st.session_state['file_type'] = 'Combined'
            else:
                st.session_state['file_type'] = 'Unknown'
            
            # Display file type prominently instead of warning about missing columns
            if 'file_type' in st.session_state:
                file_type = st.session_state['file_type']
                if file_type == 'TA':
                    st.success("📊 TA ESCALATIONS DATA")
                elif file_type == 'Census':
                    st.success("📊 CENSUS ESCALATIONS DATA")
                elif file_type == 'Combined':
                    st.success("📊 COMBINED ESCALATIONS DATA")
                else:
                    st.warning("⚠️ UNKNOWN ESCALATIONS DATA TYPE")
                    
                # Still check for missing required columns but don't show warnings for subject line variants
                critical_headers = [h for h in expected_headers if h not in ["Subject line (Manual TA Escalation)", "Subject line (Manual census Escalation)"]]
                missing_critical = [header for header in critical_headers if header not in df.columns]
                if missing_critical:
                    st.warning(f"Warning: The following important columns are missing: {', '.join(missing_critical)}")
        ...
        with tab3:  # Data Explorer Tab
            ...
            with filter_col3:
                ...
                if 'file_type' in st.session_state:
                    file_type = st.session_state['file_type']
                    if file_type == 'TA':
                        st.info("📄 TA Escalations File Detected")
                    elif file_type == 'Census':
                        st.info("📄 Census Escalations File Detected")
                    elif file_type == 'Combined':
                        st.info("📄 Combined Escalations File Detected")
            ...
            # Handle subject search for both TA and Census escalations
            if search_subject:
                # Create a mask for subject search that works with either column
                subject_mask = pd.Series(False, index=filtered_explorer_df.index)
                
                # Check TA subject line if it exists
                if "Subject line (Manual TA Escalation)" in df.columns:
                    ta_mask = filtered_explorer_df["Subject line (Manual TA Escalation)"].str.contains(search_subject, case=False, na=False)
                    subject_mask = subject_mask | ta_mask
                    
                # Check Census subject line if it exists
                if "Subject line (Manual census Escalation)" in df.columns:
                    census_mask = filtered_explorer_df["Subject line (Manual census Escalation)"].str.contains(search_subject, case=False, na=False)
                    subject_mask = subject_mask | census_mask
                    
                # Apply the combined mask
                if subject_mask.any():
                    filtered_explorer_df = filtered_explorer_df[subject_mask]
